                ...
                <h2 class="login-header w3_header">Log in</h2>
				    ...
				    <div class="w3l_grid">
                        <form class="login-container" >
                             ...
                             <input type="button" id="login_submit" value="Submit" style="width: 350px; height: 40px; margin-top: 20px" >
                        </form>
					...
					<script type="text/javascript">
						$(document).ready(function() {
							$("#login_submit").click(function(){
								...
								if (email == "") {
									$("#error1").empty();
									$("#error1").append("<i>Không để trống email</i>");
									
								}else $("#error1").remove();
								
								if (password == "") {
									$("#error2").empty();
									$("#error2").append("<i>Không để trống password</i>");
								} else{
									$("#error2").remove();
									$.ajax({
									  url: "/uyenttt/login/check",
									  method: "GET",
									  ...
									  success : function(response){
									  	if (response == "error") {
											alert("email hoặc mật khẩu không đúng")
									  		...
									  		window.location.replace("/uyenttt/indexadmin");
									  	}
									  	else {
									  		window.location.replace("/uyenttt/");
									  	}
									  		}
									});
								}
